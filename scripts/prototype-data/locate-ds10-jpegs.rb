...
CSV.open output_file, 'w+', headers: true do |csv|
  ...
  DEPENDENT_ON_DS.each do |inst|
    ...
    Dir["#{inst_dir}/mets/*.xml"].each do |mets_xml|
      ...
      row                       = {}
      row[:inst]                = inst
      row[:callno]              = DS::Extractor::DS10.extract_shelfmark xml
      row[:folio]               = 'None'
      ...
      pages.each do |page_node|
        ...
        DS::Extractor::DS10.extract_filenames(page_node).each do |filename|
          row[:mets_image_filename] = filename
          row[:jpeg]                = 'NO_FILE' # default value
          row[:folio]               = DS::Extractor::DS10.extract_folio_num page_node
