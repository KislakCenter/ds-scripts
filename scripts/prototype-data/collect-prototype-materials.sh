...
# Convert CSV format
${SCRIPT_DIR}/../../bin/ds-recon materials "${recon_opts[@]}" -a legacy -t mets $files
...
do
  ${SCRIPT_DIR}/../../bin/ds-recon materials "${recon_opts[@]}" -a ${inst} -t marc ${SCRIPT_DIR}/../../data/prototype-data/${inst}/*.xml
done
...
do
  ${SCRIPT_DIR}/../../bin/ds-recon materials "${recon_opts[@]}" -a ${inst} -t tei ${SCRIPT_DIR}/../../data/prototype-data/${inst}/*.xml
done
...
CSVS=$(for x in legacy ${MARC_INSTS} ${TEI_INSTS}; do echo "${TMP_DIR}/materials-${x}.csv"; done)

ruby ${SCRIPT_DIR}/csv_cat.rb --sort --uniq -o ${TMP_DIR}/materials-combined.csv $CSVS
