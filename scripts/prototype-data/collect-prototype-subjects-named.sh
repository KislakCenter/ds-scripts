...
# Convert CSV format
${SCRIPT_DIR}/../../bin/ds-recon subjects --named-subjects "${recon_opts[@]}" -a legacy -t mets $files
...
#######################
CSVS=$(for x in legacy ${MARC_INSTS}; do echo "${TMP_DIR}/named-subjects-${x}.csv"; done)
