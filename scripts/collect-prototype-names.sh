...
# Convert CSV format
bundle exec ruby ${SCRIPT_DIR}/../bin/recon-names.rb -o ${TMP_DIR} -a legacy -t mets $files
...
do
  bundle exec ruby ${SCRIPT_DIR}/../bin/recon-names.rb -o ${TMP_DIR} -a ${inst} -t marc ${SCRIPT_DIR}/../data/prototype-data/${inst}/*.xml
done
...
do
  bundle exec ruby ${SCRIPT_DIR}/../bin/recon-names.rb -o ${TMP_DIR} -a ${inst} -t tei ${SCRIPT_DIR}/../data/prototype-data/${inst}/*.xml
done
...
CSVS=$(for x in legacy ${MARC_INSTS} ${TEI_INSTS}; do echo "names-${x}.csv"; done)

(
  cd $TMP_DIR
  ruby ${SCRIPT_DIR}/csv_cat.rb -o ./names-combined.csv $CSVS
#  # create a file with the header (=first line of legacy.csv)
#  head -1 names-legacy.csv > names-combined.csv
#  # append all but first line of all the CSVs to combined-ds.csv
#  tail -q -n +2 ${CSVS} | sort -u >> names-combined.csv
)
