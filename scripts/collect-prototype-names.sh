...
# Convert CSV format
${SCRIPT_DIR}/../bin/recon names "${recon_opts[@]}" -a legacy -t mets $files
...
do
  ${SCRIPT_DIR}/../bin/recon names -v "${recon_opts[@]}" -a ${inst} -t marc ${SCRIPT_DIR}/../data/prototype-data/${inst}/*.xml
done
...
do
  ${SCRIPT_DIR}/../bin/recon names "${recon_opts[@]}" -a ${inst} -t tei ${SCRIPT_DIR}/../data/prototype-data/${inst}/*.xml
done
