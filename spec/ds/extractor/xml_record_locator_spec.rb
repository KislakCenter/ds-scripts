...
RSpec.describe DS::Extractor::XmlRecordLocator do
  ...
  context 'tries to locate a MARC xml record' do
    let(:parsed_xml) { Nokogiri::XML(File.read(fixture_path 'marc_xml/9949533433503681_marc.xml')) }
    let(:lookup_value) { '9949533433503681' }
    let(:bad_lookup_value) { 'BAD_LOOKUP_VALUE' }
    let(:lookup_value_location) { "//record[./controlfield[@tag='001' and ./text() = 'ID_PLACEHOLDER']]" }

    it 'locates a present MARC xml record' do
      record = locator.locate_record(parsed_xml, lookup_value, lookup_value_location)
      expect(record).to be_present
    end

    it 'does not locate a record when one is not present' do
      record = locator.locate_record(parsed_xml, bad_lookup_value, lookup_value_location)
      expect(record).not_to be_present
    end
  end

  context 'tries to locate a DS METS xml record' do
    let(:parsed_xml) { Nokogiri::XML(File.read(fixture_path 'ds_mets_xml/ds_mets-nelson-atkins-kg40.xml'))}
  ...
  context 'tries to locate a TEI xml record' do
    let(:parsed_xml) { Nokogiri::XML(File.read(fixture_path 'tei_xml/lewis_o_031_TEI.xml')) }
