...
describe DS::Extractor::DsMetsXmlExtractor do
  ...
  context "notes" do
    context 'extract_ms_note' do
      it 'formats an untyped ms note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_ms_note na_ds_xml).to include 'Manuscript note: Untyped note'
      end

      it 'formats a bibliography note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_ms_note na_ds_xml).to include 'Bibliography: Bibliography'
      end
    end

    context 'extract_part_note' do
      it 'formats an untyped part note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_part_note na_ds_xml).to include 'One leaf: Untyped part note'
      end
    end

    context 'extract_text_note' do
      it 'formats an untyped text note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_text_note na_ds_xml).to include 'One leaf: Untyped text note'
      end

      it 'formats a condition note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_text_note na_ds_xml).to include 'Status of text, One leaf: Condition note'
      end

      it 'formats an incipit' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_text_note na_ds_xml).to include 'Incipit, One leaf: Text content note'
      end

      it 'formats an explicit' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_text_note na_ds_xml).to include 'Explicit, One leaf: Text abstract'
      end
    end
    ...
    context 'extract_other_name'do
      it  'returns names with role "other"' do
        actual  = DS::Extractor::DsMetsXmlExtractor.extract_other_names_as_recorded ds_names_xml
    ...
    context 'extract_docket' do
      it 'finds two dockets' do
        actual = DS::Extractor::DsMetsXmlExtractor.extract_docket ds_docket_xml
    ...
    context 'extract_page_note' do
      it 'formats an untyped page note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_page_note na_ds_xml).to include 'f. 1r: Untyped page note'
      end

      it 'formats an incipit' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_page_note na_ds_xml).to include 'Incipit, f. 1v: Page content note'
      end

      it 'formats an explicit' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_page_note na_ds_xml).to include 'Explicit, f. 1v: Page abstract'
      end
    ...
    context 'extract_note' do
      it 'formats all the notes' do
        notes = DS::Extractor::DsMetsXmlExtractor.extract_notes na_ds_xml
      ...
      it 'includes any dockets' do
        notes = DS::Extractor::DsMetsXmlExtractor.extract_notes ds_docket_xml
  ...
  context 'physical description' do
    context 'extract_ms_phys_desc' do
      it 'formats a binding note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_ms_phys_desc na_ds_xml).to include 'Binding: Not bound'
      end
    end

    context 'extract_part_phys_desc' do
      it 'formats a physical details note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_part_phys_desc na_ds_xml).to include 'Figurative details, One leaf: Physical details note'
      end

      it 'formats a physical description other decoration note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_part_phys_desc na_ds_xml).to include 'Other decoration, One leaf: Physical description note'
      end

      it 'formats a physical description number of scribes note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_part_phys_desc na_ds_xml).to include 'Number of scribes, One leaf: number of scribes'
      end

      it 'formats a script note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_part_phys_desc na_ds_xml).to include 'Script, One leaf: Script note'
      end

      it 'formats a medium note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_part_phys_desc na_ds_xml).to include 'Music, One leaf: Medium note'
      end

      it 'formats a technique note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_part_phys_desc na_ds_xml).to include 'Layout, One leaf: Technique note'
      end

      it 'formats a marks note' do
        expect(DS::Extractor::DsMetsXmlExtractor.extract_part_phys_desc na_ds_xml).to include 'Watermarks, One leaf: Marks note'
      end
    ...
    context 'extract_physical_description' do
      it 'formats all the phys desc notes' do
        descs = DS::Extractor::DsMetsXmlExtractor.extract_physical_description na_ds_xml
