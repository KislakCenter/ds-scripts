...
RSpec.describe 'DS::Manifest' do
  ...
  let(:parsed_csv) { parse_csv <<~EOF
    holding_institution_ds_qid,filename,holding_institution_wikidata_label,source_data_type,ds_id,record_lookup_value,lookup_value_location_in_source,record_last_updated,call_number,title,iiif_manifest_url,link_to_institutional_record,dated,manifest_generated_at
