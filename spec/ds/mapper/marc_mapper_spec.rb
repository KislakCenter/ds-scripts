...
RSpec.describe DS::Mapper::MarcMapper do
  ...
  context 'extract_record' do
    ...
    let(:xpath) {
      entry.lookup_value_location_in_source.gsub('ID_PLACEHOLDER', lookup_value)
    }
    ...
    it 'returns the expected record' do
      ...
      expect(record.at_xpath(xpath).to_s).to eq entry.record_lookup_value
    end
  end
    ...
    let(:expected_map) {
      {
        ...
        :uniform_title_agr                  => "|",
        :title_as_recorded                  => "Periermenias Aristotelis ... [etc.|Peri hermēneias",
