    ...
    module DS10
      module ClassMethods
        ...
        def extract_author xml
          DS::Extractor::DS10.extract_name xml, *%w{ author [author] }
        end

        def extract_artist xml
          DS::Extractor::DS10.extract_name xml, *%w{ artist [artist] illuminator }
        end

        def extract_scribe xml
          DS::Extractor::DS10.extract_name xml, *%w{ scribe [scribe] }
        end

        def extract_other_name xml
          DS::Extractor::DS10.extract_name(xml, 'other').flat_map { |n| n.split %r{\s*;\s*} }
        end
        ...
        def extract_recon_splits xml
          ...
          data = []
          data += DS::Extractor::DS10.extract_ownership xml, lookup_split: false
